...
for i in num_layers:
	...
	for j in temp_store:
		...
		start = time.time()
		history = new_model.fit_generator(train_generator, validation_data=valid_generator, epochs=20, callbacks=[early_callback])
	...
	# log the reults in the log dataframe
		...
		log_df.to_csv(os.path.join("AutoFC_ResNet", "AutoFC_ResNet_log_randomsearch_Caltech101.csv"))
